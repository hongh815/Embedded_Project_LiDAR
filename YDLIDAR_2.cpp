...
void directionSet(float degree) {
    ...
    switch(out_result) {
        ...
        case 76:
            ...
            break;
        default:
            cout << "dir : DEFAULT(config FORWARD) - " << STOP << endl;
...
float return_function_two(float degree, float distance) {
    ...
    cout << "send degree ::>>" << send_degree << endl;
...
//    send_degree = 1;
    ...
    if(digitalRead(ARDUINO)){
        printf("doit\n");
    }else cout << "NO ARDUINO READ DATA ERROR ;;;" << endl;//for debug
    send_dist = -1;
   
    if(digitalRead(ARDUINO)==HIGH && fa == 1){ // this add if no fa (flag) this code no operation; frist signal must operating; 
...
//        sleep(2);
        ...
        for(int i = 0; i < 720; i++) {
            ...
            if(distance > 0.15 && distance < 7.99){ //block dis < 0.4
