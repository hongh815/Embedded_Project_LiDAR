...
//    send_degree = 1;
    ...
    if(digitalRead(ARDUINO)==1){
        
        printf("doit\n");
        boolflag = true;
    }else {
        cout << "NO ARDUINO READ DATA ERROR ;;;" << endl;
        boolflag = false;
        return; 
    }//for debug
    send_dist = -1;
   
    if(boolflag == true){ // this add if no fa (flag) this code no operation; frist signal must operating;         
//        sleep(2);
        for(int i = 0; i < 720; i++) {
